...
Aplica lógica de mayoría solo para localidades de Bogotá.
"""
...
class EnhancedMapVisualizerBogotaMajority:
    """
    Genera visualizaciones de mapas con lógica de mayoría para Bogotá.
    """
    ...
    def create_enhanced_map(self, assignments_file: str, output_file: str):
        ...
        # Para localidades de Bogotá: contar IPS por KAM
        ...
        for kam_id, ips_list in assignments.items():
            for ips in ips_list:
                ...
                if not ips.get('is_population_only', False):
                    ...
                    # Para localidades de Bogotá, contar por KAM
                ...
                # Para localidades, NO agregar automáticamente, decidir por mayoría
                if 'localityid' in ips and ips['localityid']:
                    # Se decidirá más adelante por mayoría
                    pass
        
        # Determinar KAM dominante para localidades de Bogotá
        locality_dominant_kam = {}
        locality_sharing_info = {}
        ...
        for locality_id, kam_counts in locality_kam_counts.items():
            if kam_counts:
                ...
                locality_dominant_kam[locality_id] = dominant_kam
                
                # Agregar la localidad al territorio del KAM dominante
                kam_territories[dominant_kam].add(locality_id)
                
                # Si hay más de un KAM, es territorio compartido
                if len(sorted_kams) > 1:
                    total_ips = sum(count for _, count in sorted_kams)
                    locality_sharing_info[locality_id] = {
                        'kams': sorted_kams,
                        'total_ips': total_ips,
        ...
        if locality_sharing_info:
            print(f"   ✓ {len(locality_sharing_info)} localidades de Bogotá compartidas")
        ...
        # Dibujar localidades con lógica de mayoría
        ...
        if os.path.exists(localities_path):
            for filename in os.listdir(localities_path):
                if filename.endswith('.geojson'):
                    ...
                    # Para localidades, usar el KAM dominante
                    if locality_id in locality_dominant_kam:
                        assigned_kam = locality_dominant_kam[locality_id]
                        color = kam_colors[assigned_kam]
                        kam_name = kam_info[assigned_kam]['name']
                        has_ips = True
                        opacity = 0.5
                    ...
                    try:
                        ...
                        if has_ips:
                            ...
                            if locality_id in locality_sharing_info:
                                info = locality_sharing_info[locality_id]
                        ...
                        is_shared = locality_id in locality_sharing_info
    ...
    def _add_legend(self, m, kam_colors, kam_info, assignments):
        ...
        <div style="font-size: 10px; color: #666; font-style: italic;">
            Localidades de Bogotá se colorean<br>
            según el KAM con más IPS
        </div>
        </div>
        '''
