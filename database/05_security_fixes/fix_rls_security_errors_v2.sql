-- Script para corregir errores de seguridad RLS en Supabase (Versión 2 - Corregida)
-- Fecha: 2025-01-07
...
-- Vista: visit_statistics (corregida - sin visit_status)
...
DO $$
DECLARE
  has_end_date_func BOOLEAN;
  has_pdf_url_func BOOLEAN;
BEGIN
  -- Verificar si existe la función get_contract_end_date
  SELECT EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'get_contract_end_date'
    AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
  ) INTO has_end_date_func;
  
  -- Verificar si existe la función get_contract_pdf_url
  SELECT EXISTS (
    SELECT 1 FROM pg_proc 
    WHERE proname = 'get_contract_pdf_url'
    AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
  ) INTO has_pdf_url_func;
  
  -- Crear la vista según las funciones disponibles
  IF has_end_date_func AND has_pdf_url_func THEN
    EXECUTE 'CREATE VIEW public.hospital_contracts_view AS
    SELECT 
      hc.*,
      h.name AS hospital_name,
      h.municipality_name,
  ...
  ELSE
    ...
    SELECT 
      ...
      h.municipality_name,
      h.department_name,
      hc.end_date AS calculated_end_date,
      NULL::text AS pdf_url
    FROM public.hospital_contracts hc
    JOIN public.hospitals h ON hc.hospital_id = h.id';
  END IF;
...
-- Mostrar vistas recreadas
SELECT 
  'Vista recreada: ' || viewname AS vistas_sin_security_definer
FROM pg_views
WHERE schemaname = 'public'
  AND viewname IN ('visit_statistics', 'kam_statistics', 'territory_statistics', 'hospital_contracts_view', 'territory_assignments');