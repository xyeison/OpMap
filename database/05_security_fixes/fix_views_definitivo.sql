-- ============================================
-- SCRIPT DEFINITIVO PARA RECREAR VISTAS SIN SECURITY DEFINER
...
SELECT 
  ...
  v.kam_id,
  v.kam_name,
  v.observations,  -- Campo correcto (no 'notes')
...
-- Lista explícita de campos para evitar duplicación
...
SELECT 
  ...
  h.municipality_name,
  h.department_name,
  -- Usar las funciones si existen
  COALESCE(get_contract_end_date(hc.id), hc.end_date) AS calculated_end_date,
  COALESCE(get_contract_pdf_url(hc.id), hc.pdf_url) AS calculated_pdf_url
...
SELECT 
  ...
  '5 vistas recreadas exitosamente' as resultado
UNION ALL
SELECT 
  'ERRORES RESUELTOS',
  'Todas las vistas ahora están sin SECURITY DEFINER'
UNION ALL
SELECT 
  'NOTA',
  'El error de spatial_ref_sys es normal (tabla del sistema PostGIS)';