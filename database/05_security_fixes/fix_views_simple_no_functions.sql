-- ============================================
-- SCRIPT SIMPLIFICADO PARA RECREAR VISTAS SIN SECURITY DEFINER
-- Sin usar funciones que puedan no existir
-- ============================================
...
forced_data AS (
  SELECT DISTINCT ON (fa.territory_id)
    fa.territory_id,
  ...
  ORDER BY fa.territory_id, fa.updated_at DESC
),
...
-- ============================================
-- 4. RECREAR VISTA: hospital_contracts_view (SIMPLIFICADA)
-- ============================================
DROP VIEW IF EXISTS public.hospital_contracts_view CASCADE;

-- Versión simplificada sin funciones
...
SELECT 
  ...
  h.municipality_name,
  h.department_name,
  -- Calcular end_date basado en start_date + duration_months si end_date es null
  COALESCE(
    hc.end_date, 
    CASE 
      WHEN hc.duration_months IS NOT NULL 
      THEN hc.start_date + (hc.duration_months || ' months')::interval
      ELSE NULL
    END
  ) AS calculated_end_date,
  hc.pdf_url AS calculated_pdf_url  -- Simplemente usar el campo existente
...
SELECT 
  ...
  'Todas las vistas recreadas sin SECURITY DEFINER y sin dependencias de funciones' as detalle;