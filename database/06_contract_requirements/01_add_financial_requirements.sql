-- ==================================================================
-- AGREGAR REQUISITOS FINANCIEROS A CONTRATOS/OPORTUNIDADES
-- ==================================================================
-- Este script agrega campos para especificar los indicadores financieros
-- requeridos por cada contrato y permite validar el cumplimiento
-- ==================================================================

-- 1. AGREGAR CAMPOS DE REQUISITOS FINANCIEROS A hospital_contracts
-- ==================================================================
ALTER TABLE hospital_contracts
ADD COLUMN IF NOT EXISTS requisitos_financieros jsonb DEFAULT '{}'::jsonb,
ADD COLUMN IF NOT EXISTS indice_liquidez_requerido numeric(10,2),
ADD COLUMN IF NOT EXISTS indice_endeudamiento_maximo numeric(10,2),
ADD COLUMN IF NOT EXISTS cobertura_intereses_minimo numeric(10,2),
ADD COLUMN IF NOT EXISTS patrimonio_minimo numeric(15,2),
ADD COLUMN IF NOT EXISTS capital_trabajo_minimo numeric(15,2),
ADD COLUMN IF NOT EXISTS experiencia_anos_minimo integer,
ADD COLUMN IF NOT EXISTS facturacion_anual_minima numeric(15,2),
ADD COLUMN IF NOT EXISTS rentabilidad_minima numeric(10,2),
ADD COLUMN IF NOT EXISTS otros_requisitos text,
ADD COLUMN IF NOT EXISTS cumple_requisitos boolean,
ADD COLUMN IF NOT EXISTS fecha_validacion_requisitos timestamp with time zone,
ADD COLUMN IF NOT EXISTS notas_cumplimiento text;

-- Agregar comentarios
COMMENT ON COLUMN hospital_contracts.requisitos_financieros IS 'Requisitos financieros específicos en formato JSON';
COMMENT ON COLUMN hospital_contracts.indice_liquidez_requerido IS 'Índice de liquidez mínimo requerido (ej: 1.2)';
COMMENT ON COLUMN hospital_contracts.indice_endeudamiento_maximo IS 'Índice de endeudamiento máximo permitido (ej: 0.7 = 70%)';
COMMENT ON COLUMN hospital_contracts.cobertura_intereses_minimo IS 'Cobertura de intereses mínima (ej: 1.5)';
COMMENT ON COLUMN hospital_contracts.patrimonio_minimo IS 'Patrimonio mínimo requerido en millones COP';
COMMENT ON COLUMN hospital_contracts.capital_trabajo_minimo IS 'Capital de trabajo mínimo en millones COP';
COMMENT ON COLUMN hospital_contracts.experiencia_anos_minimo IS 'Años de experiencia mínimos requeridos';
COMMENT ON COLUMN hospital_contracts.facturacion_anual_minima IS 'Facturación anual mínima en millones COP';
COMMENT ON COLUMN hospital_contracts.rentabilidad_minima IS 'Rentabilidad mínima sobre ventas (%)';
COMMENT ON COLUMN hospital_contracts.cumple_requisitos IS 'Indica si mi empresa cumple con los requisitos';
COMMENT ON COLUMN hospital_contracts.fecha_validacion_requisitos IS 'Última fecha de validación de requisitos';

-- 2. CREAR TABLA PARA CONFIGURACIÓN DE MI EMPRESA
-- ==================================================================
...
-- 3. CREAR FUNCIÓN PARA ACTUALIZAR INDICADORES DE MI EMPRESA
-- ==================================================================
...
-- 4. CREAR FUNCIÓN PARA VALIDAR CUMPLIMIENTO DE REQUISITOS
-- ==================================================================
...
-- 5. CREAR VISTA PARA DASHBOARD DE CUMPLIMIENTO
-- ==================================================================
...
COMMENT ON VIEW v_cumplimiento_contratos IS 'Vista para dashboard de cumplimiento de requisitos por contrato';

-- 6. INSERTAR CONFIGURACIÓN INICIAL DE MI EMPRESA (EJEMPLO)
-- ==================================================================
...
-- 7. AGREGAR ALGUNOS REQUISITOS DE EJEMPLO A CONTRATOS
-- ==================================================================
UPDATE hospital_contracts
SET 
    indice_liquidez_requerido = 1.2,
    indice_endeudamiento_maximo = 0.7,
    cobertura_intereses_minimo = 1.5,
    patrimonio_minimo = 5000,
    capital_trabajo_minimo = 2000
WHERE id IN (
    SELECT id 
    FROM hospital_contracts 
    WHERE active = true 
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ SISTEMA DE REQUISITOS FINANCIEROS CREADO';
    RAISE NOTICE '========================================';
    RAISE NOTICE '';
    RAISE NOTICE 'Funcionalidades agregadas:';
    RAISE NOTICE '1. Campos de requisitos en contratos';
    RAISE NOTICE '2. Tabla de configuración de mi empresa';
    RAISE NOTICE '3. Cálculo automático de indicadores';
    RAISE NOTICE '4. Validación de cumplimiento';
    RAISE NOTICE '5. Vista para dashboard';
    RAISE NOTICE '';
    RAISE NOTICE 'Próximos pasos:';
    RAISE NOTICE '1. Configurar los datos de tu empresa';
    RAISE NOTICE '2. Agregar requisitos a cada contrato';
    RAISE NOTICE '3. Revisar el dashboard de cumplimiento';
    RAISE NOTICE '';
END $$;
